...
# Creating an instance of the conditions database's API
api_factory = APIFactory()
conditionsDB = api_factory.construct_DB_API("../conditionsDatabase/config.yml")

# Benchmarking: calculate time required for retrieving daniel data from conditions database
...
logger.info(msg='Daniel%15.6f%13.6f' % (wall_time_end - wall_time_start, cpu_time_end - cpu_time_start))

# Benchmarking: calculate time required for retrieving survey data from conditions database
wall_time_start = time.time()
cpu_time_start = time.clock()
...
cpu_time_end = time.clock()
wall_time_end = time.time()
logger.info(msg='survey%15.6f%13.6f' % (wall_time_end - wall_time_start, cpu_time_end - cpu_time_start))
...
if withCorrections:
    # Benchmarking: calculate time required for retrieving alignCorrection data from conditions database
    wall_time_start = time.time()
    cpu_time_start = time.clock()
...
def makeAlignmentConstantsPersistent():
    ...
    # Benchmarking: calculate time required for adding alignConstants data to the database
    wall_time_start = time.time()
    cpu_time_start = time.clock()
    conditionsDB.add_condition("muflux/driftTubes", "alignConstants", "TUE2019", loads(dumps(alignConstants)), "alignment")
    cpu_time_end = time.clock()
    wall_time_end = time.time()
    # Record time difference to the log as an execution performance benchmarking
...
def importAlignmentConstants():
    ...
    if not sTree.GetCurrentFile().Get('alignConstants') or withCorrections:
        ...
        return
    try:
        # Benchmarking: calculate time required for getting alignConstants condition data from the database
        wall_time_start = time.time()
        cpu_time_start = time.clock()
...
def importRTrel():
    # Benchmarking: calculate time required for importing RTrelations from Pickl
    wall_time_start = time.time()
    cpu_time_start = time.clock()
    for fname in fnames:
...
def init(database='muflux_RTrelations.pkl',remake=False,withReco=False):
    ...
    if not RTrelations.has_key(rname) or remake:
        ...
        # Benchmarking: calculate time required for writing RTrelations to Pickl
        wall_time_start = time.time()
        cpu_time_start = time.clock()
...
if options.command == "":
    ...
    # Benchmarking: calculate time required for running the script by passing no arguments as command
    cpu_time_start = time.clock()
    wall_time_start = time.time()
    ...
    if sTree.GetBranch('MCTrack'):
        ...
        # Benchmarking: calculate time required to import the Pickl data
        wall_time_start2 = time.time()
        cpu_time_start2 = time.clock()
        RTrelations = pickle.load(open(database))
        cpu_time_end2 = time.clock()
        wall_time_end2 = time.time()
        logger.info(msg='loading_RTrelations%15.6f%13.6f' % (wall_time_end2 - wall_time_start2, cpu_time_end2 - cpu_time_start2))
...
if options.command == "recoStep0":
    # Benchmarking: calculate time required for running the script by passing recoStep0 as command argument
    cpu_time_start = time.clock()
    wall_time_start = time.time()
    withTDC=False
    ...
    # benchmarking: calculate time required for running the script by passing recoStep1 as command argument
    cpu_time_start = time.clock()
    wall_time_start = time.time()
    if sTree.GetBranch('MCTrack'):
        MCdata = True
    ...
    # Benchmarking: calculate time required for running the script by passing recoStep2 as command argument
    cpu_time_start = time.clock()
    wall_time_start = time.time()
    if sTree.GetBranch('MCTrack'):
        MCdata = True
    ...
    # Benchmarking: calculate time required for running the script by passing anaResiduals as command argument
    cpu_time_start = time.clock()
    wall_time_start = time.time()
    ...
    # Benchmarking: calculate time required for running the script by passing alignment as command argument
    cpu_time_start = time.clock()
    wall_time_start = time.time()
    ROOT.gROOT.SetBatch(True)
    if sTree.GetBranch('MCTrack'):
    ...
    # Benchmarking: calculate time required for running the script by passing plotResiduals as command argument
    cpu_time_start = time.clock()
    wall_time_start = time.time()
    ...
    # Benchmarking: calculate time required for running the script by passing recoMuonTaggerTracks as command argument
    cpu_time_start = time.clock()
    wall_time_start = time.time()
    importAlignmentConstants()
    ...
    wall_time_end = time.time()
    logger.info(
        msg='recoMuonTaggerTracks%15.6f%13.6f' % (wall_time_end - wall_time_start, cpu_time_end - cpu_time_start))
elif options.command == "momResolution":
    # Benchmarking: calculate time required for running the script by passing momResolution as command argument
    cpu_time_start = time.clock()
    wall_time_start = time.time()
    MCdata = True
    withDefaultAlignment = True
    ...
    # Benchmarking: calculate time required for running the script by passing test as command argument
    cpu_time_start = time.clock()
    wall_time_start = time.time()
